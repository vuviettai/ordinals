...
use diesel::{associations::HasTable, PgConnection};
...
use crate::{
  index::reorg::Reorg,
  runebeta::{SatRange, Statistic, Table},
  schema::{indexing_block_timestamps, statistics},
  settings::Settings,
  timestamp, Result, Rune, SHUTTING_DOWN,
};

use super::{
  connection::WriteTransaction, models::NewIndexingBlockTimestamp, OutPointValue,
  RuneBetaIndex as Index,
};
use {
  self::{inscription_updater::InscriptionUpdater, rune_updater::RuneUpdater},
  crate::index::{fetcher::Fetcher, *},
...
impl<'index> Updater<'index> {
  pub(crate) fn update_index<'a>(&'a mut self, &mut wtx: WriteTransaction) -> Result {
    let start = Instant::now();
    let starting_height = u32::try_from(self.index.client.get_block_count()?).unwrap() + 1;
    let starting_index_height = self.height;

    wtx
      .open_table(indexing_block_timestamps::table::defaut())?
      .insert(&NewIndexingBlockTimestamp {
  ...
  fn index_block(
    ...
    let mut statistics = wtx.open_table(statistics::table::default())?;
    ...
    if self.index.index_runes && self.height >= self.index.settings.first_rune_height() {
      let mut outpoint_to_rune_balances = |&mut connection: PgConnection| {};
      ...
      let mut rune_updater = RuneUpdater::new(
        self.height,
        Rune::minimum_at_height(self.index.settings.chain(), Height(self.height)),
        runes,
        block.header.time,
      );
