...
use crate::{
  ...
  runebeta::{
    ...
    models::{
      NewContentTypeCount, NewInscriptionEntry, NewSatpoint, NewTransaction, NewTransactionIn,
      NewTransactionOut,
    },
  },
  schema::{content_type_counts, inscription_entries, inscriptions, satpoints, transactions},
  unbound_outpoint, Chain, InscriptionId,
};

use self::{
  entry::{InscriptionEntry, InscriptionIdValue},
  event::Event,
};
...
pub(super) struct InscriptionUpdater<'a, 'db, 'tx> {
  ...
  pub(super) content_type_to_count: Table<'db, 'tx, content_type_counts::table>,
  pub(super) cursed_inscription_count: u64,
  pub(super) event_sender: Option<&'a Sender<Event>>,
  pub(super) flotsam: Vec<Flotsam>,
  pub(super) height: u32,
  pub(super) home_inscription_count: u64,
  pub(super) home_inscriptions: Table<'db, 'tx, inscriptions::table>,
  ...
  // pub(super) transaction_id_to_transaction: Table<'db, 'tx, transactions::table>,
  pub(super) transactions: Table<'db, 'tx, transactions::table>,
  ...
  pub(super) inscription_entry: Table<'db, 'tx, inscription_entries::table>,
  //pub(super) sequence_number_to_satpoint: &'a mut Table<'db, 'tx, u32, &'static SatPointValue>,
  pub(super) satpoints: Table<'db, 'tx, satpoints::table>,
...
impl<'a, 'db, 'tx> InscriptionUpdater<'a, 'db, 'tx> {
  pub fn new(
    ...
    event_sender: Option<&'a Sender<Event>>,
    flotsam: Vec<Flotsam>,
    height: u32,
    ...
    Self {
      ...
      chain,
      content_type_to_count: wtx.open_table(content_type_counts::table::default()),
      cursed_inscription_count,
      event_sender,
      flotsam,
      height,
      home_inscription_count,
      home_inscriptions: wtx.open_table(inscriptions::table::default()),
      //id_to_sequence_number: todo!(),
      index_transactions: todo!(),
      //inscription_number_to_sequence_number: todo!(),
      lost_sats,
      next_sequence_number,
      //outpoint_to_value: todo!(),
      reward,
      transactions: wtx.open_table(transactions::table::default()),
      //sat_to_sequence_number: todo!(),
      //satpoint_to_sequence_number: todo!(),
      //sequence_number_to_children: todo!(),
      inscription_entry: wtx.open_table(inscription_entries::table::default()),
      //sequence_number_to_satpoint: todo!(),
      satpoints: wtx.open_table(satpoints::table::default()),
      timestamp,
  ...
  pub(super) fn index_inscriptions(
    ...
    for (input_index, tx_in) in tx.input.iter().enumerate() {
      ...
      while let Some(inscription) = envelopes.peek() {
        ...
        let content_type = inscription
          .payload
          .content_type
          .map(|v| format!("{:X}", v.as_slice()));
        let content_type_count = self
          .content_type_to_count
          .get(content_type)?
          .map(|entry| entry.value())
          .unwrap_or_default();
        let new_content_type_count = NewContentTypeCount {
          content_type,
          count: content_type_count + 1,
        };
        self.content_type_to_count.insert(&new_content_type_count)?;
        // self
        //   .content_type_to_count
    ...
    if self.index_transactions && inscriptions {
      ...
      let mut new_transaction_ins = output
        .iter()
        ...
        .collect();
      let mut new_transaction_outs = input
        .iter()
        .map(|txin| NewTransactionIn {
          tx_hash: txid.to_string(),
          ...
          sequence_number: txin.sequence.0,
          witness: format!("{:02X}", txin.witness.to_vec().as_slice()),
        })
        .collect();
      self.transactions.insert(&new_transaction)?;
      self.transaction_ins.insert(&new_transaction_ins)?;
      self.transaction_outs.insert(&new_transaction_outs)?;
  ...
  fn update_inscription_location(
    ...
    let (unbound, sequence_number) = match flotsam.origin {
      ...
      Origin::New {
        ...
        /*
         * TaiVV: Mar 15, 2024
         * When store indexing data into postgres, this map is already store in the inscription_entries
         */
        // self
        ...
        let new_inscription_entry = NewInscriptionEntry {
          charms,
          fee,
          height: self.height,
          tx_hash: inscription_id.txid.to_string(),
          inscription_index: inscription_id.index,
          inscription_number,
        ...
        self.inscription_entry.insert(&new_inscription_entry);
        ...
        //   .store(),
        // )?;
        // This mapping already in the inscription_entry
        // self
        //   .id_to_sequence_number
    ...
    let new_satpoint_record = NewSatpoint {
      sequence_number,
      tx_hash: satpoint.outpoint.txid.to_string(),
      vout: satpoint.outpoint.vout,
      sat_offset: satpoint.offset,
    };
    self.satpoints.insert(&new_satpoint_record)?;
    // self
    //   .satpoint_to_sequence_number
